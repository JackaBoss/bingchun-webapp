...
function makeTokens(user) {
  const payload = { id: user.id, phone: user.phone, tier: user.tier };
...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    const [rows]  = await db.query('SELECT id, phone, name, points, tier FROM users WHERE id = ?', [userId]);
    const user    = rows[0];
...
router.post('/login', async (req, res) => {
  ...
  try {
    const [rows] = await db.query(
      'SELECT id, phone, name, password_hash, points, tier, is_active FROM users WHERE phone = ?',
      [phone]
    );
...
router.post('/refresh', async (req, res) => {
  ...
  try {
    ...
    const [userRows] = await db.query(
      'SELECT id, phone, name, points, tier FROM users WHERE id = ?',
      [payload.id]
    );
...
router.get('/me', requireAuth, async (req, res) => {
  const [rows] = await db.query(
    'SELECT id, phone, email, name, points, tier, created_at FROM users WHERE id = ?',
    [req.user.id]
  );
  res.json(rows[0]);
});
