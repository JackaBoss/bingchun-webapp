...
// GET — staff and manager can read
  ...
  try {
    ...
    const [rows] = await db.query(
      `SELECT v.*, m.name AS free_item_name, u.name AS created_by_name
       ...
       ${where} ORDER BY v.${sortCol} ${sortDir}`
    );
    res.json(rows);
  } catch (err) {
    console.error(err);
...
// GET redemptions — staff and manager
  ...
  try {
    const [rows] = await db.query(
      `SELECT vr.*, u.name AS member_name, u.phone AS member_phone, o.order_no
       ...
       WHERE vr.voucher_id = ? ORDER BY vr.redeemed_at DESC`,
      [req.params.id]
    );
    res.json(rows);
  } catch (err) {
...
// POST — manager only
router.post('/admin/vouchers', requireManager, async (req, res) => {
  ...
  try {
    const [result] = await db.query(
      `INSERT INTO vouchers (code, description, type, value, free_item_id, min_spend, max_discount,
...
// PATCH — manager only
router.patch('/admin/vouchers/:id', requireManager, async (req, res) => {
...
// PATCH toggle — manager only
router.patch('/admin/vouchers/:id/toggle', requireManager, async (req, res) => {
  try {
...
router.post('/vouchers/validate', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (voucher.min_spend > 0 && orderSubtotal < voucher.min_spend)
      return res.status(400).json({ valid: false, message: `Minimum spend of RM${parseFloat(voucher.min_spend).toFixed(2)} required` });

    let discountAmount = 0; let label = '';
    ...
    res.json({
      valid: true, voucher_id: voucher.id, type: voucher.type, value: voucher.value,
      ...
      label, message: `Voucher applied: ${label}`,
    });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Server error' });
...
router.post('/vouchers/redeem', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (!voucher || !voucher.is_active) { await conn.rollback(); return res.status(400).json({ error: 'Voucher no longer valid' }); }
    if (voucher.total_uses !== null && voucher.used_count >= voucher.total_uses) { await conn.rollback(); return res.status(400).json({ error: 'Voucher limit reached' }); }
    await conn.query('UPDATE vouchers SET used_count = used_count + 1 WHERE id = ?', [voucher_id]);
    await conn.query(
      `INSERT INTO voucher_redemptions (voucher_id, user_id, order_id, discount_applied) VALUES (?, ?, ?, ?)`,
