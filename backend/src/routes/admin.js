...
router.get('/members', requireAdmin, async (req, res) => {
  try {
    const [members] = await db.query(
      `SELECT id, name, phone, points, tier, created_at FROM users WHERE role = 'member' ORDER BY created_at DESC`
    );
    res.json(members);
  } catch (err) {
    console.error(err);
...
router.post('/credit-points', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const walkinSaleId = wsResult.insertId;

    await conn.query(
      `INSERT INTO points_transactions (user_id, type, amount, balance, note)
       VALUES (?, 'earn', ?, ?, ?)`,
...
// ── POST /api/admin/walkin-sales/:id/items ────────────────────────────────
router.post('/walkin-sales/:id/items', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    await db.query(
      `INSERT INTO walkin_sale_items (walkin_sale_id, menu_item_id, item_name, quantity, unit_price) VALUES ?`,
      [rows]
    );
  ...
  try {
    ...
    for (const sale of sales) {
      const [items] = await db.query(
        'SELECT * FROM walkin_sale_items WHERE walkin_sale_id = ?', [sale.id]
      );
      sale.items = items;
    }
...
// ── GET /api/admin/members/:id/item-ranking ───────────────────────────────
  ...
  try {
    ...
    const [[stats]] = await db.query(
      `SELECT
         ...
         SUM(bill_amount) AS total_spend,
         AVG(bill_amount) AS avg_bill,
         MAX(created_at)  AS last_visit,
         MIN(created_at)  AS first_visit
       FROM walkin_sales WHERE user_id = ?`,
      [req.params.id]
    );
