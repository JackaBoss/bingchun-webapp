...
router.get('/outlets', requireManager, async (req, res) => {
  try {
    ...
    const params = [];
    if (req.user.outlet_id) { sql += ' WHERE id = ?'; params.push(req.user.outlet_id); }
  ...
  try {
    ...
    const [todaySales] = await db.query(
      `SELECT COUNT(*) as order_count, COALESCE(SUM(total),0) as revenue
       ...
       ${scope.where ? 'AND ' + scope.where : ''}`, scope.params
    );
    ...
    const [recentOrders] = await db.query(
      `SELECT o.*, ot.name as outlet_name, ot.store_code,
       ...
       ORDER BY o.created_at DESC LIMIT 20`, scope.params
    );
    res.json({ today: todaySales[0], total_users: totalUsers.count, pending_orders: pendingOrders[0].count, recent_orders: recentOrders });
  } catch (err) { console.error(err); res.status(500).json({ error: 'Server error' }); }
});
...
router.get('/orders', requireAdmin, async (req, res) => {
  ...
  let sql = `SELECT o.*, ot.name as outlet_name, ot.store_code,
             u.name as customer_name, u.phone as customer_phone
             FROM orders o JOIN outlets ot ON o.outlet_id = ot.id JOIN users u ON o.user_id = u.id WHERE 1=1`;
  const params = [];
...
router.get('/orders/:id', requireAdmin, async (req, res) => {
  try {
    ...
    const [[order]] = await db.query(
      `SELECT o.*, ot.name as outlet_name, ot.store_code, u.name as customer_name, u.phone as customer_phone
...
router.patch('/orders/:id/status', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    if (['paid', 'ready', 'cancelled'].includes(status)) {
      ...
      notifyStatusChange({ order: updated, status, outletName: outlet?.name || '', customerName: customer?.name || '' })
...
// ── PATCH /api/admin/menu/:id — manager only (price/name/description) ────
router.patch('/menu/:id', requireManager, async (req, res) => {
...
// ── PATCH /api/admin/menu/:id/toggle — staff + manager (availability only) ─
...
router.post('/members', requireManager, async (req, res) => {
  ...
  try {
    ...
    const [result] = await db.query(
      `INSERT INTO users (name, phone, email, password_hash, tier, points, role, is_active) VALUES (?, ?, ?, ?, ?, ?, 'member', 1)`,
  ...
  try {
    ...
    const [rows] = await db.query(
      `SELECT u.id, u.name, u.phone, u.role, u.is_active, u.outlet_id, u.created_at,
       ...
       ORDER BY u.role DESC, u.name ASC`, scope.params
    );
    res.json(rows);
  } catch (err) { console.error(err); res.status(500).json({ error: 'Server error' }); }
});
...
router.post('/staff', requireManager, async (req, res) => {
  ...
  try {
    const [[existing]] = await db.query('SELECT id, role FROM users WHERE phone = ?', [phone]);

    if (existing) {
      // ── Existing member → promote to staff ──────────────────────────
      if (existing.role === 'member') {
        // Password is required when promoting a member (they need a staff password)
        if (!password) return res.status(400).json({ error: 'password is required when promoting a member to staff' });
        const hash = await bcrypt.hash(password, 12);
        await db.query(
          'UPDATE users SET role = ?, outlet_id = ?, password_hash = ?, is_active = 1 WHERE id = ?',
          [role, effectiveOutletId, hash, existing.id]
        );
        const [[updated]] = await db.query(
          `SELECT u.id, u.name, u.phone, u.role, u.is_active, u.outlet_id, u.created_at,
                  ot.name as outlet_name, ot.store_code
        ...
        return res.status(200).json({ ...updated, _promoted: true });
      }
      // Already staff/manager
      return res.status(409).json({ error: 'Phone number is already registered as staff' });
    }

    // ── Brand new staff account ──────────────────────────────────────
    if (!name || !password) return res.status(400).json({ error: 'name and password are required for new staff' });
    const hash = await bcrypt.hash(password, 12);
    const [result] = await db.query(
      `INSERT INTO users (name, phone, password_hash, role, outlet_id, is_active, email_verified) VALUES (?, ?, ?, ?, ?, 1, 1)`,
  ...
  try {
    const [rows] = await db.query(
      `SELECT ws.id, ws.walkin_order_no, ws.bill_amount, ws.points_earned, ws.staff_note, ws.created_at,
       ...
       FROM walkin_sales ws JOIN users u ON ws.user_id = u.id
       ...
       GROUP BY ws.id ORDER BY ws.created_at DESC LIMIT 200`, scope.params
    );
    if (rows.length) {
  ...
  try {
    ...
    const [sales] = await db.query(
      `SELECT ws.*, u.name as staff_name, ot.name as outlet_name
       FROM walkin_sales ws LEFT JOIN users u ON ws.staff_id = u.id LEFT JOIN outlets ot ON ws.outlet_id = ot.id
       WHERE ws.user_id = ? ${scope.where ? 'AND ' + scope.where : ''} ORDER BY ws.created_at DESC`,
      [req.params.id, ...scope.params]
    );
...
router.get('/members/:id/item-ranking', requireManager, async (req, res) => {
  try {
    const [[member]] = await db.query('SELECT id, name, phone FROM users WHERE id = ? AND role = "member"', [req.params.id]);
    if (!member) return res.status(404).json({ error: 'Member not found' });
    const scope = outletScope(req.user, 'ws');
    const [ranking] = await db.query(
      `SELECT wsi.menu_item_id, wsi.item_name, SUM(wsi.quantity) AS total_qty,
              SUM(wsi.quantity * wsi.unit_price) AS total_spend,
              COUNT(DISTINCT wsi.walkin_sale_id) AS order_count, MAX(ws.created_at) AS last_ordered
       FROM walkin_sale_items wsi JOIN walkin_sales ws ON wsi.walkin_sale_id = ws.id
       WHERE ws.user_id = ? ${scope.where ? 'AND ' + scope.where : ''}
       GROUP BY wsi.menu_item_id, wsi.item_name ORDER BY total_qty DESC, total_spend DESC LIMIT 5`,
      [req.params.id, ...scope.params]
    );
  ...
  try {
    const [orders] = await db.query(
      `SELECT o.id, o.order_no, o.total, o.notes, o.created_at,
       ...
       ${scope.where ? 'AND ' + scope.where : ''} ORDER BY o.created_at ASC`, scope.params
    );
    for (const order of orders) {
      const [items] = await db.query(
        `SELECT oi.item_name, oi.quantity, oi.unit_price, oi.notes,
         ...
         FROM order_items oi LEFT JOIN order_item_options oio ON oio.order_item_id = oi.id
         WHERE oi.order_id = ? GROUP BY oi.id`, [order.id]
      );
      order.items = items;
    }
    res.json(orders);
  ...
  try {
    const [summary] = await db.query(
      `SELECT COUNT(*) AS order_count, SUM(o.subtotal) AS gross_sales, SUM(o.discount) AS total_discounts,
       ...
       ${scope.where ? 'AND ' + scope.where : ''}`, [from, to, ...scope.params]
    );
    const [daily] = await db.query(
      `SELECT DATE(o.created_at) AS date, ot.name AS outlet, COUNT(*) AS orders, SUM(o.total) AS revenue
       ...
       ${scope.where ? 'AND ' + scope.where : ''}
       GROUP BY DATE(o.created_at), o.outlet_id ORDER BY date ASC`, [from, to, ...scope.params]
    );
    const [topItems] = await db.query(
      `SELECT oi.item_name, SUM(oi.quantity) AS qty_sold, SUM(oi.quantity * oi.unit_price) AS revenue
       ...
       WHERE DATE(o.created_at) BETWEEN ? AND ? AND o.status NOT IN ('cancelled')
       ${scope.where ? 'AND ' + scope.where : ''}
       GROUP BY oi.item_name ORDER BY qty_sold DESC LIMIT 20`, [from, to, ...scope.params]
    );
    const [orders] = await db.query(
      `SELECT o.order_no, DATE(o.created_at) AS date, TIME(o.created_at) AS time,
       ...
       FROM orders o JOIN users u ON o.user_id = u.id JOIN outlets ot ON o.outlet_id = ot.id
       WHERE DATE(o.created_at) BETWEEN ? AND ? AND o.status NOT IN ('cancelled')
       ${scope.where ? 'AND ' + scope.where : ''} ORDER BY o.created_at DESC`, [from, to, ...scope.params]
    );
    if (format === 'csv') {
      const rows = [
