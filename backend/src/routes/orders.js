...
// Points config — 1 point per RM1 spent, redeem at RM0.01/point
...
// Generate a readable order number: BC-20250325-0042
...
router.post('/', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const [[user]] = await conn.query(
      'SELECT id, points FROM users WHERE id = ? FOR UPDATE', [req.user.id]
    );

    // 2. Calculate subtotal from DB prices (never trust client prices)
    let subtotal = 0;
    const lineItems = [];
    ...
    for (const cartItem of items) {
      ...
      for (const optionId of (cartItem.option_ids || [])) {
        const [[opt]] = await conn.query(
          'SELECT id, label, extra_price FROM item_options WHERE id = ?', [optionId]
        );
    ...
    const [orderResult] = await conn.query(
      `INSERT INTO orders
        (user_id, outlet_id, order_no, subtotal, discount, total,
         points_earned, points_redeemed, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [user.id, outlet_id, orderNo, subtotal, discount, total,
       pointsEarned, pointsRedeemed, notes || null]
    );
    const orderId = orderResult.insertId;

    // 5. Insert line items + options
    for (const line of lineItems) {
      ...
      for (const opt of line.options) {
        await conn.query(
          `INSERT INTO order_item_options (order_item_id, option_id, label, extra_price)
           VALUES (?, ?, ?, ?)`,
    ...
    // 6. Deduct redeemed points, will add earned after payment
    if (pointsRedeemed > 0) {
  ...
  try {
    ...
    for (const item of items) {
      const [opts] = await db.query(
        'SELECT * FROM order_item_options WHERE order_item_id = ?', [item.id]
      );
      item.options = opts;
    }
