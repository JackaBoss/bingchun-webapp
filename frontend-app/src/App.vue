    ...
    <template v-if="auth.isLoggedIn">
      ...
      <div class="main-wrap">
        <header class="topbar">
          ...
          <!-- Language toggle on mobile topbar -->
          <div class="lang-toggle-mobile">
            <button
              v-for="l in langs"
              :key="l.code"
              :class="['lang-btn-sm', { active: lang === l.code }]"
              @click="setLang(l.code)"
            >{{ l.label }}</button>
          </div>
        </header>
...
.forbidden-toast {
  background: #fee2e2; color: #991b1b; padding: 10px 20px;
  font-size: 13px; font-weight: 600; border-bottom: 1px solid #fecaca;
