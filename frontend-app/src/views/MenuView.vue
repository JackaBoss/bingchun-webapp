  ...
  <div class="menu-page">
    ...
    <div class="coming-soon-banner">
      ...
      <div class="banner-text">
        <div class="banner-title">Online ordering coming very soon!</div>
        <div class="banner-sub">Browse our menu below — visit us in-store to order today.</div>
      </div>
    </div>
    ...
    <div v-else-if="error" class="state-wrap">
      ...
      <p>Couldn't load menu. Try again later.</p>
    </div>
          ...
          >
            <div class="item-img-wrap">
              <img
                ...
                @error="onImgError"
              />
              <div v-if="!item.is_available" class="unavail-overlay">Unavailable</div>
            </div>
            <div class="item-info">
              <div class="item-name">{{ item.name }}</div>
              <div v-if="item.name_zh" class="item-name-zh">{{ item.name_zh }}</div>
              <div v-if="item.description" class="item-desc">{{ item.description }}</div>
              <div class="item-footer">
                <span class="item-price">RM {{ Number(item.base_price).toFixed(2) }}</span>
                <!-- ADD TO CART INTENTIONALLY OMITTED — restore when ordering goes live -->
              </div>
            </div>
          </div>
        </div>
...
const OUTLETS = [
  { id: 1, name: 'Amerin Mall' },
  { id: 2, name: 'Mantin' },
]

const outlets        = ref(OUTLETS)
const selectedOutlet = ref(OUTLETS[0].id)
const categories     = ref([])
const loading        = ref(true)
const error          = ref(false)
const activeCat      = ref(null)
const catTabsEl      = ref(null)
