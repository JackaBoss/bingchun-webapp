<template>
  <div class="profile-page">
    ...
    <div class="profile-header">
      ...
      <div :class="['tier-badge', `tier-${auth.tier}`]">{{ tierLabel }}</div>
    </div>
    ...
    <div class="points-card card">
      ...
      <div class="tier-progress" v-else>
        <p style="font-size:13px;color:#ffd700;font-weight:600;">{{ t('gold_member') }}</p>
      </div>
    </div>

    <!-- QR Code card -->
    <div class="card section-card qr-card">
      <p class="section-label">{{ t('qr_label') }}</p>
      <p class="qr-hint">{{ t('qr_hint') }}</p>
      <div class="qr-wrap">
    ...
    <div class="card section-card" v-if="pointsData">
      <p class="section-label">{{ t('points_history') }}</p>
      <div v-if="pointsData.transactions.length === 0" class="empty-tx">
        <p style="font-size:32px;margin-bottom:8px">⭐</p>
        <p>{{ t('no_transactions') }}</p>
        <p style="font-size:12px;color:var(--text-muted);margin-top:4px">{{ t('no_tx_sub') }}</p>
      </div>
...
const tierMap = {
  bronze: { nextKey: 'tier_silver', threshold: 500 },
  silver: { nextKey: 'tier_gold',   threshold: 2000 },
}
const tierLabel     = computed(() => t.value(`tier_${auth.tier}`) || auth.tier)
const nextTierLabel = computed(() => t.value(tierMap[auth.tier]?.nextKey) || '')
...
function txLabel(type) {
  return {
    earn:   t.value('tx_earn'),
    redeem: t.value('tx_redeem'),
    expire: t.value('tx_expire'),
    adjust: t.value('tx_adjust'),
  }[type] || type
}
...
onMounted(async () => {
  try {
    ...
    if (pointsData.value) auth.user.points = pointsData.value.balance
...
.profile-header { background: var(--blue); color: #fff; padding: 28px 20px 24px; display: flex; align-items: center; gap: 14px; }
.avatar { width: 52px; height: 52px; border-radius: 50%; background: rgba(255,255,255,0.25); display: flex; align-items: center; justify-content: center; font-size: 20px; font-weight: 700; }
.profile-info { flex: 1; }
.profile-info h2 { font-size: 18px; font-weight: 700; }
.profile-info p { font-size: 13px; opacity: 0.8; margin-top: 2px; }
.tier-badge { padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 700; text-transform: uppercase; }
...
.qr-hint   { font-size: 13px; color: var(--text-muted); margin-bottom: 16px; line-height: 1.4; }
.qr-wrap   { display: flex; justify-content: center; background: #fff; border-radius: 12px; padding: 16px; border: 1.5px solid var(--border); }
.qr-canvas { border-radius: 6px; }
.qr-phone  { text-align: center; font-size: 18px; font-weight: 700; letter-spacing: 2px; margin-top: 12px; color: var(--text); }
...
.empty-tx { font-size: 14px; color: var(--text-muted); text-align: center; padding: 20px 0; }
...
.action-row { display: flex; justify-content: space-between; align-items: center; width: 100%; padding: 14px 0; background: none; border: none; border-bottom: 1px solid var(--border); font-size: 15px; cursor: pointer; color: var(--text); }
